...
import { SlackAPI } from 'slack-API/mod.ts'
import { SlackAPIClient } from 'slack-API/types.ts'

import { ErrorDetail, RequestBody } from './types.ts'
import { markdownTemplateBody, markdownTemplateHeader } from './templates/slack.ts'
...
export const authenticationFailed = async (
  ...
  const errorDetail: ErrorDetail = {
    failureHost: ips(req),
    failureDetails,
    failedRequestDetails: requestBody ? JSON.stringify(requestBody, null, 2) : '',
