...
import {
  conversationHistory,
  conversations, conversationsAssignees, conversationsAssigneesHistory, conversationsUsers, organizations,
  teams,
  users,
...
describe(
  ...
  () => {
    it("closed", async () => {
      ...
      assertEquals(org[0].id, conversationCLosedRequest.conversation.organization.id);
      assertEquals(org[0].name, conversationCLosedRequest.conversation.organization.name);
      ...
      assertEquals(newTeams[0].id, conversationCLosedRequest.conversation.team!.id);
      assertEquals(newTeams[0].name, conversationCLosedRequest.conversation.team!.name);
      assertEquals(newTeams[0].organizationId, conversationCLosedRequest.conversation.team!.organization);
    });
    ...
    it("upsert", async () => {
      await req(JSON.stringify(conversationReopenedRequest));
      const newRequest = JSON.parse(JSON.stringify(conversationReopenedRequest));
    ...
    it("assignee change", async () => {
      ...
      assertEquals(assignees[0].userId, conversationAssigneeChangeRequest.conversation.assignees[0].id);
      assertEquals(assignees[0].unassigned, conversationAssigneeChangeRequest.conversation.assignees[0].unassigned);
      assertEquals(assignees[0].closed, conversationAssigneeChangeRequest.conversation.assignees[0].closed);
      assertEquals(assignees[0].archived, conversationAssigneeChangeRequest.conversation.assignees[0].archived);
      assertEquals(assignees[0].trashed, conversationAssigneeChangeRequest.conversation.assignees[0].trashed);
      assertEquals(assignees[0].junked, conversationAssigneeChangeRequest.conversation.assignees[0].junked);
      assertEquals(assignees[0].assigned, conversationAssigneeChangeRequest.conversation.assignees[0].assigned);
      assertEquals(assignees[0].flagged, conversationAssigneeChangeRequest.conversation.assignees[0].flagged);
      assertEquals(assignees[0].snoozed, conversationAssigneeChangeRequest.conversation.assignees[0].snoozed);

      const assigneeHistory = await db.select().from(conversationsAssigneesHistory)
